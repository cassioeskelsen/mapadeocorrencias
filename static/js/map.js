$(function() {
  ...
  var tiles3geo = "http://tiles.3geo.com.br";
 
   var path = $(document.body).data().path,
      loader = $('<img/>').attr('src', '/static/images/loader.gif').addClass('loader'),
      po = org.polymaps,
      lightstyle = 5870,
      darkstyle  = 1960,
      svg = po.svg('svg'),
      ...
      .center({lat: -26.504682, lon: -49.091034})   
      .zoom(12)
      .zoomRange([1,18])
      .add(po.interact())
      .add(po.hash())
    ...
    map.add(po.image()
      .url(po.url("http://tiles.3geo.com.br"
      + "/tiles/osm" +  "/{Z}/{X}/{Y}.png")));
 ...
 * Generate trimet routes
   ...
   var onPointClick = function(event) {
    ...
    mapel.maptip(this)
      ...
      .top(function(tip) {
        ...
        if(ctype.length == 0) {
          $.getJSON('/delito/' + d.codigo + '/.json', function(data) {

            otype = data.code;
            hdr.append($('<span/>').addClass('badge').text('E').attr('data-code', otype))
              .append($('<a/>').attr('href', '/delito/' + data.codigo).text(data.nome))
              .append(close)
              .addClass(otype)
          })
         ctype = $('#sbar h1').clone()
        ...
        var time = Date.parse(props.reportado).toString('dd/MM/yyyy  -  HH:mm')
        time = $('<a />')
            .attr('href', '/crimes/' + props._id)
            .text(time)
            
        bdy.text(props.endereco)
        bdy.append($('<span />')
          .addClass('date')
          .html(time))
  ...
  var resizer = $('<span />').addClass('resizer').text('`'),
      ...
      pnav   = $('#offense-nav'),
      logo   = $('<img />')
        .attr('src', '/static/images/logo-small.png')
        .addClass('fslogo'),
